...
export const createRazorpayOrder = createServerFn({ method: "POST" }).handler(async ({ data }) => {
  ...
  async ({ data }) => {
    ...
    try {
      ...
      await notifyAllAdmins(
        `💰 Successful Razorpay Deposit: ${username} added ₹${order.amount}`,
        "/admin/deposits",
      );
    } catch (e) {
...
export const getWalletBalance = createServerFn({ method: "POST" }).handler(async () => {
  const userObj = await getCurrentUser();
  const db = await getDb();
  const userId = userObj.id;
