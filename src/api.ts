...
export const loginUser = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const user = userStmt.get(username, password) as any;
...
export const signupUser = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const result = insertStmt.run(username, password, ign || null, uid || null, email || null, phone || null);
...
export const getUserFromSession = createServerFn({ method: "GET" })
  .handler(async ({ data }) => {
    ...
    `);
    const user = stmt.get(sessionId, new Date().toISOString()) as any;
    return user || null;
  });
...
export const getUsers = createServerFn({ method: "GET" })
  .handler(async () => {
    const { db } = await import("./lib/db");
    return db.prepare('SELECT id, username, password, role, created_at, deposit_balance, winning_balance FROM users').all();
  });
...
export const addTournament = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(title, game, mode, format, entry, prize, slots, filled, startsAt, status, banner, room_id || null, room_pass || null);
    return { success: true };
  });
...
export const updateTournament = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const old = db.prepare('SELECT room_id, room_pass FROM tournaments WHERE id = ?').get(id) as any;
    ...
    const stmt = db.prepare(`
      ...
      WHERE id=?
    `);
    stmt.run(title, game, mode, format, entry, prize, slots, filled, startsAt, status, banner, room_id || null, room_pass || null, id);

    if ((room_id && room_id !== old?.room_id) || (room_pass && room_pass !== old?.room_pass)) {
      const registrations = db.prepare('SELECT user_id FROM registrations WHERE tournament_id = ?').all(id) as any[];
      ...
      for (const r of registrations) {
        insertNotif.run(r.user_id, notifMsg.trim());
      }
    }
...
export const toggleHeroTournament = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    const { db } = await import("./lib/db");
    const id = data as unknown as number;
    const current = db.prepare('SELECT is_hero FROM tournaments WHERE id = ?').get(id) as any;
    db.prepare('UPDATE tournaments SET is_hero = ? WHERE id = ?').run(current.is_hero ? 0 : 1, id);
    return { success: true };
  });
...
export const registerForTournament = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const t = db.prepare('SELECT filled, slots, entry FROM tournaments WHERE id = ?').get(tournamentId) as any;
    if (!t) throw new Error("Tournament not found");
    if (t.filled >= t.slots) throw new Error("Tournament is full");

    // Check if already registered
    const existing = db.prepare('SELECT id FROM registrations WHERE user_id = ? AND tournament_id = ?').get(userId, tournamentId);
    if (existing) throw new Error("You are already registered for this tournament");

    db.transaction(() => {
      // Handle Entry Fee Deduction
      if (t.entry > 0) {
        const user = db.prepare('SELECT deposit_balance, winning_balance FROM users WHERE id = ?').get(userId) as any;
        ...
        db.prepare('UPDATE users SET deposit_balance = ?, winning_balance = ? WHERE id = ?').run(newDeposit, newWinning, userId);
      }
      ...
      if (teamName) {
        const teamInfo = db.prepare('SELECT id, leader_id FROM teams WHERE name = ?').get(teamName) as any;
        if (teamInfo) {
          teamId = teamInfo.id;
      ...
      if (needsApproval) {
        ...
        const requester = db.prepare('SELECT username FROM users WHERE id = ?').get(userId) as any;
        const tourney = db.prepare('SELECT title FROM tournaments WHERE id = ?').get(tournamentId) as any;

        db.prepare('INSERT INTO notifications (user_id, message, action_type, action_data) VALUES (?, ?, ?, ?)').run(
          leaderId,
        ...
        db.prepare('UPDATE tournaments SET filled = filled + 1 WHERE id = ?').run(tournamentId);

        // Give notification to team members
        const team = db.prepare('SELECT id FROM teams WHERE leader_id = ?').get(userId) as any;
        if (team) {
          const members = db.prepare('SELECT user_id FROM team_members WHERE team_id = ? AND user_id IS NOT NULL').all(team.id) as any[];
          const insertNotif = db.prepare('INSERT INTO notifications (user_id, message) VALUES (?, ?)');
          for (const m of members) {
            if (m.user_id !== userId) {
              insertNotif.run(m.user_id, `🏆 Your Captain has registered your team for a new tournament! Check your matches.`);
            }
          }
        }
      }
...
export const getProfile = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    return db.prepare('SELECT id, username, role, ign, uid, email, phone, created_at, deposit_balance, winning_balance FROM users WHERE id = ?').get(userId) as any;
  });
...
export const updateCoinBalance = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.prepare(`UPDATE users SET ${type} = ? WHERE id = ?`).run(amount, userId);
    return { success: true };
  });
...
export const getMyTeam = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    const { db } = await import("./lib/db");
    const userId = data as unknown as number;
    let team = db.prepare('SELECT * FROM teams WHERE leader_id = ?').get(userId) as any;
    
    if (!team) {
      const member = db.prepare('SELECT team_id FROM team_members WHERE user_id = ?').get(userId) as any;
      if (member) {
        team = db.prepare('SELECT * FROM teams WHERE id = ?').get(member.team_id) as any;
      }
    }
    
    if (!team) return null;
    const members = db.prepare('SELECT * FROM team_members WHERE team_id = ?').all(team.id);
    const leader = db.prepare('SELECT username, ign, uid FROM users WHERE id = ?').get(team.leader_id);
...
export const saveMyTeam = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      let team = db.prepare('SELECT id FROM teams WHERE leader_id = ?').get(userId) as any;
      let teamId;
      let existingMembers: any[] = [];
      
      if (team) {
        teamId = team.id;
        existingMembers = db.prepare('SELECT uid, user_id FROM team_members WHERE team_id = ? AND user_id IS NOT NULL').all(teamId) as any[];
        db.prepare('UPDATE teams SET name = ?, logo = ? WHERE id = ?').run(teamName, logo || '', teamId);
        db.prepare('DELETE FROM team_members WHERE team_id = ?').run(teamId);
      } else {
        const res = db.prepare('INSERT INTO teams (name, leader_id, logo) VALUES (?, ?, ?)').run(teamName, userId, logo || '');
        teamId = res.lastInsertRowid;
      }

      const insertMember = db.prepare('INSERT INTO team_members (team_id, user_id, ign, uid, role) VALUES (?, ?, ?, ?, ?)');
      ...
      for (const m of members) {
        if (m.ign && m.uid) {
          ...
          if (!memberUserId) {
             const req = db.prepare("SELECT user_id FROM team_requests WHERE team_id = ? AND uid = ? AND status = 'approved' ORDER BY created_at DESC LIMIT 1").get(teamId, m.uid) as any;
             if (req) memberUserId = req.user_id;
          }

          insertMember.run(teamId, memberUserId || null, m.ign, m.uid, m.role || 'player');
        }
      }
      ...
      if (existingMembers.length > 0) {
        ...
        for (const ex of existingMembers) {
          if (!newUids.has(ex.uid)) {
            insertNotif.run(ex.user_id, `❌ You have been removed from the team ${teamName}.`);
          }
        }
      }
    })();
...
export const leaveTeam = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      const user = db.prepare('SELECT username, ign FROM users WHERE id = ?').get(userId) as any;
      const team = db.prepare('SELECT name, leader_id FROM teams WHERE id = ?').get(teamId) as any;
      ...
      db.prepare('INSERT INTO notifications (user_id, message) VALUES (?, ?)').run(team.leader_id, `⚠️ ${user.ign || user.username} has left your team ${team.name}.`);
    })();
    return { success: true };
  });
...
export const deleteTeam = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      ...
      const members = db.prepare('SELECT user_id FROM team_members WHERE team_id = ? AND user_id IS NOT NULL').all(teamId) as any[];
      ...
      for (const m of members) {
        if (m.user_id !== userId) {
          insertNotif.run(m.user_id, `⚠️ The team ${team.name} has been deleted by the captain.`);
        }
      }
    })();
    return { success: true };
...
export const getAllTeams = createServerFn({ method: "GET" })
  .handler(async () => {
    const { db } = await import("./lib/db");
    const teams = db.prepare('SELECT * FROM teams ORDER BY created_at DESC').all() as any[];
    const allMembers = db.prepare('SELECT * FROM team_members').all() as any[];
...
export const requestJoinTeam = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const existing = db.prepare("SELECT id FROM team_requests WHERE team_id = ? AND user_id = ? AND status = 'pending'").get(teamId, userId);
...
export const getTeamRequests = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const team = db.prepare('SELECT id FROM teams WHERE leader_id = ?').get(leaderId) as any;
    if (!team) return [];
...
export const resolveTeamRequest = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      const req = db.prepare('SELECT * FROM team_requests WHERE id = ?').get(requestId) as any;
      if (!req) throw new Error("Request not found");
      
      db.prepare('UPDATE team_requests SET status = ? WHERE id = ?').run(status, requestId);
      
      if (status === 'approved') {
        const teamCount = db.prepare('SELECT COUNT(*) as count FROM team_members WHERE team_id = ?').get(req.team_id) as any;
        ...
        db.prepare('INSERT INTO team_members (team_id, user_id, ign, uid, role) VALUES (?, ?, ?, ?, ?)')
          .run(req.team_id, req.user_id, req.ign, req.uid, 'player');
          
        db.prepare('INSERT INTO notifications (user_id, message) VALUES (?, ?)').run(req.user_id, '🎉 Your request to join the team has been approved!');
      } else {
        db.prepare('INSERT INTO notifications (user_id, message) VALUES (?, ?)').run(req.user_id, '❌ Your request to join the team was declined.');
      }
    })();
    return { success: true };
  });
...
export const getMyMatches = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    const leaderTeam = db.prepare('SELECT name, id FROM teams WHERE leader_id = ?').get(userId) as any;
...
export const saveTournamentResults = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      const tourney = db.prepare('SELECT title, prize, mode FROM tournaments WHERE id = ?').get(tournamentId) as any;
      if (!tourney) throw new Error("Tournament not found");

      const stmt = db.prepare('UPDATE registrations SET kills = ?, position = ?, points = ? WHERE id = ? AND tournament_id = ?');
      const insertNotif = db.prepare('INSERT INTO notifications (user_id, message) VALUES (?, ?)');
      const addPrize = db.prepare('UPDATE users SET winning_balance = winning_balance + ? WHERE id = ?');
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        stmt.run(r.killsNum, r.matchPosition, r.calculatedPoints, r.id, tournamentId);
        ...
        if (r.calculatedPoints > 0 || r.matchPosition > 0) {
           insertNotif.run(r.user_id, `🏆 Match Results! You scored ${r.calculatedPoints} points (${r.killsNum} kills, Match Position ${r.matchPosition}) and finished Rank #${overallRank} in your recent tournament.`);
        }
      }
...
export const rescheduleTournament = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      const tourney = db.prepare('SELECT title FROM tournaments WHERE id = ?').get(id) as any;
      if (!tourney) throw new Error("Tournament not found");

      db.prepare("UPDATE tournaments SET status = 'upcoming' WHERE id = ?").run(id);
      db.prepare("UPDATE registrations SET kills = 0, position = 0, points = 0 WHERE tournament_id = ?").run(id);
      ...
      for (const r of registrations) {
        insertNotif.run(r.user_id, notifMsg);
      }
    })();
    return { success: true };
  });
...
export const getGlobalLeaderboard = createServerFn({ method: "GET" })
  .handler(async () => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!t) {
        const t2 = db.prepare('SELECT name FROM teams WHERE leader_id = ?').get(rows[i].user_id) as any;
...
export const resolveTournamentRequest = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      const req = db.prepare('SELECT * FROM tournament_requests WHERE id = ?').get(requestId) as any;
      if (!req) throw new Error("Request not found");
      if (req.status !== 'pending') throw new Error("Request already resolved");

      db.prepare("UPDATE tournament_requests SET status = ? WHERE id = ?").run(status, requestId);
      db.prepare("UPDATE notifications SET action_type = 'resolved' WHERE action_type = 'tournament_request' AND action_data = ?").run(requestId.toString());

      const tourney = db.prepare('SELECT title, entry FROM tournaments WHERE id = ?').get(req.tournament_id) as any;
      const insertNotif = db.prepare('INSERT INTO notifications (user_id, message) VALUES (?, ?)');
      ...
      if (status === 'approved') {
        ...
        db.prepare('UPDATE tournaments SET filled = filled + 1 WHERE id = ?').run(req.tournament_id);

        insertNotif.run(req.requested_by, `🎉 Your Captain has approved the tournament registration for ${tourney.title}!`);

        const members = db.prepare('SELECT user_id FROM team_members WHERE team_id = ? AND user_id IS NOT NULL').all(req.team_id) as any[];
        for (const m of members) {
          if (m.user_id !== req.requested_by) {
            insertNotif.run(m.user_id, `🏆 Your Captain has registered your team for ${tourney.title}! Check your matches.`);
          }
        }
      } else {
        if (tourney.entry > 0) {
...
export const processWithdrawal = createServerFn({ method: "POST" })
  .handler(async ({ data }) => {
    ...
    db.transaction(() => {
      const user = db.prepare('SELECT winning_balance FROM users WHERE id = ?').get(userId) as any;
      if (!user) throw new Error("User not found");
      ...
      db.prepare('INSERT INTO withdrawals (user_id, amount, upi_id, upi_number) VALUES (?, ?, ?, ?)').run(userId, amount, upiId, upiNumber);
