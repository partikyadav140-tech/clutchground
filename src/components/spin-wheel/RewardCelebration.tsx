...
export function RewardCelebration({ open, onClose, prize }: RewardCelebrationProps) {
  ...
  useEffect(() => {
    ...
    const spawnExplosion = (cx: number, cy: number, count = 50) => {
    ...
    const spawnCannon = (x: number, y: number, angleDeg: number, count = 30) => {
    ...
    // Initial triggers - optimized count for smoothness
    spawnExplosion(width / 2, height / 2 - 100, 60);
    setTimeout(() => spawnCannon(0, height, -45, 30), 200);
    setTimeout(() => spawnCannon(width, height, -135, 30), 200);
    setTimeout(() => spawnExplosion(width / 2, height / 2 - 100, 20), 600);
    ...
    const updateAndDraw = () => {
      ...
      // Keep spawning minor trickles of confetti from top randomly, capped for smoothness
  ...
  const isWin = prize ? prize.amount > 0 : false;
      ...
      {open && prize && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="absolute inset-0 bg-[#090b11]/92"
          />
