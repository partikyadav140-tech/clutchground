...
import { ArrowLeft, Sparkles, Check, Lock, Crown, ShieldCheck, Zap, Star } from "lucide-react";
...
import { getProfile, getProfileShop, purchaseProfileCosmetic, updateProfile } from "@/api";
...
function ProfileShopPage() {
  ...
  const currentRegistry =
    activeTab === "dp" ? DP_ANIMATIONS : activeTab === "banners" ? BANNER_PRESETS : PROFILE_EFFECTS;

  const currentItems = useMemo(() => {
    const dbKey = activeTab === "dp" ? "frames" : activeTab === "banners" ? "banners" : "effects";
    ...
    const extraItems = dbItems
      .filter(
        (d: any) => !currentRegistry.some((local) => local.value === d.value || local.id === d.id),
      )
      .map((d: any) => ({
        id: d.id,
        value: d.value,
  ...
  const filteredAnims =
    filter === "all" ? currentItems : currentItems.filter((a) => a.tier === filter);
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="bg-card border-b border-border/50 px-4 pt-5 pb-4">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          {activeTab === "dp" &&
            "Add glowing, pulsating, and animated color effects around your profile picture!"}
          {activeTab === "banners" &&
            "Change the moving gradient background behind your profile card!"}
          {activeTab === "effects" &&
            "Overlay animated visual effects like flames, lightning, or sparkles on your profile!"}
        </p>
      </div>
              ...
              <button
                ...
                style={
                  isActive
                    ? {
                        background:
                          t === "all" ? "var(--gradient-primary)" : tc?.color || "var(--primary)",
                      }
                    : {}
                }
              >
              ...
              return (
                ...
                >
                  <div className="flex gap-3 p-3">
                    ...
                    {activeTab === "dp" ? (
                      <div
                        className="relative flex items-center justify-center shrink-0 bg-black/40 border border-border/40 rounded-2xl overflow-hidden"
                        style={{ width: 90, height: 90 }}
                      >
                        <div
                          className={`relative w-14 h-14 rounded-full flex items-center justify-center ${anim.value === "none" ? "" : ANIMATION_CLASS[anim.value] || ""}`}
                        >
                      ...
                      <div
                        className={`relative shrink-0 border border-border/40 rounded-2xl overflow-hidden ${anim.value === "default" ? "profile-banner-default" : `profile-banner-${anim.value}`}`}
                        style={{ width: 90, height: 90 }}
                      >
