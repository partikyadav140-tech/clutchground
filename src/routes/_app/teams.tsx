    ...
    <div className="bg-background min-h-screen pb-24">
      ...
      <div className="bg-white rounded-b-[2rem] shadow-[0_4px_24px_oklch(0_0_0/0.04)] pt-6 pb-6 px-4 relative overflow-hidden z-10">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1 font-semibold">Join forces and conquer</p>
        </div>

        <div className="mt-6 space-y-3">
          <Button 
            className="w-full h-12 rounded-xl font-bold bg-primary text-white shadow-primary"
            onClick={() => { if(user) router.navigate({ to: "/profile" }); else router.navigate({ to: "/login" }); }}
          >
      ...
      <div className="px-4 mt-8">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredTeams.map((t, i) => (
            ...
            >
              <div className="absolute top-0 right-0 p-4 font-display font-black text-xl text-secondary group-hover:text-primary/10 transition-colors z-0">#{t.id}</div>
              ...
              <div className="relative z-10 flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-display text-xl font-black text-foreground group-hover:text-primary transition-colors">{t.name}</h3>
                  <div className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Squad</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div className="bg-secondary/30 rounded-xl p-3 border border-border/50 flex flex-col items-center">
                  ...
                  <div className="font-display font-black text-lg text-foreground leading-tight">{t.members?.length + 1 || 1}/4</div>
                  <div className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold">Roster</div>
                </div>
                <div className="bg-secondary/30 rounded-xl p-3 border border-border/50 flex flex-col items-center">
                  <Calendar className="w-4 h-4 text-primary mb-1" />
                  <div className="font-display font-black text-sm text-foreground mt-0.5 leading-tight">{new Date(t.created_at).toLocaleDateString([], { month: 'short', year: 'numeric' })}</div>
                  <div className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold mt-0.5">Founded</div>
                </div>
              </div>
        ...
        {filteredTeams.length === 0 && (
          <div className="text-center py-16 bg-white rounded-[1.5rem] border border-border shadow-sm mt-4">
            ...
            <h3 className="font-display font-black text-lg text-foreground mb-1">No Squads Found</h3>
            <p className="text-sm text-muted-foreground max-w-[200px] mx-auto font-semibold">
              {teams.length === 0 ? "No squads have been formed yet. Be the first to create one!" : "No squads match your search query."}
            </p>
          </div>
        )}
      </div>
