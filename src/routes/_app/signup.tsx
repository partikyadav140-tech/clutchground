...
function SignupPage() {
  ...
  const handleEmailCheck = async (emailVal: string) => {
    ...
    try {
      const res = await (checkEmailExists as any)({ data: { email: trimmed } });
      if (res.exists) {
        setEmailError("This email address is already linked to an account.");
      } else {
        setEmailError("");
      }
    } catch (e) {
  ...
  const handleStep1 = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const check = await (checkEmailExists as any)({ data: { email: form.email.trim() } });
      if (check.exists) {
        setEmailError("This email address is already linked to an account.");
        toast.error("Email is already linked to an account");
        setLoading(false);
        return;
      }

      const res = await (sendEmailOtp as any)({
        data: { email: form.email.trim(), purpose: "signup" },
      });
      setOtpId(res.otpId);
      ...
      >
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-xs text-muted-foreground font-semibold px-2">
            {step === 1
              ? "Join India's Free Fire esports arena"
              : step === 2
          ...
          {step === 3 && (
            ...
            >
              ...
              <AppInput
                ...
                value={form.uid}
                onChange={(e) => setForm((f) => ({ ...f, uid: e.target.value.replace(/\D/g, "") }))}
                inputMode="numeric"
                maxLength={12}
              />
              ...
              <label className="flex items-start gap-3 px-1 pt-1">
                <input
                  type="checkbox"
                  required
                  className="accent-primary mt-0.5 w-4 h-4 shrink-0 rounded border-border"
                />
        ...
        <p className="text-center text-xs text-muted-foreground font-semibold mt-6">
          ...
          <Link
            to="/login"
            className="font-black text-primary hover:text-primary/80 transition-colors uppercase tracking-wider text-[11px]"
          >
            Sign In
          </Link>
        </p>
