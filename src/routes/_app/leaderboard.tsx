...
function LeaderboardPage() {
  const { leaderboard, settings } = Route.useLoaderData() as {
    leaderboard: any[];
  ...
  const myEntry = user
    ? leaderboard.find(
        (p: any) => p.user_id === user.id || (p.member_ids && p.member_ids.includes(user.id)),
      )
    : null;
  ...
  const rankConfig: Record<
    number,
    { medal: string; color: string; glow: string; bg: string; size: string }
  > = {
    1: {
      medal: "🥇",
      color: "#f59e0b",
      ...
      <div className="px-4 mb-5" id="tutorial-leaderboard-prize">
        <div className="relative overflow-hidden rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4 flex items-start gap-3 shadow-sm shadow-amber-500/5">
          ...
          <div className="flex-1">
            <h4 className="font-display font-black text-[11px] text-amber-500 uppercase tracking-widest mb-1">
              Weekly Championship Reward
            </h4>
            <p className="text-xs text-muted-foreground font-semibold leading-relaxed">
              The team or player securing the{" "}
              <span className="text-amber-500 font-bold">1st position</span> on the leaderboard at
              the weekly reset (Sunday 11:59 PM) will be rewarded with{" "}
              <span className="text-amber-500 font-bold">{prizeAmount} CG Coins</span>!
            </p>
          </div>
        </div>
      </div>
      ...
      {top3.length >= 3 && (
        <div className="px-4 mb-6">
          <div className="bg-card rounded-3xl border border-border p-5 shadow-card relative overflow-hidden">
            ...
            <div
              className="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-20 blur-3xl rounded-full"
      ...
      {myEntry && (
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className="w-10 h-10 rounded-2xl flex items-center justify-center font-display font-black text-sm text-foreground shrink-0"
              style={{
        ...
        {leaderboard.length === 0 ? (
          <div className="py-16 text-center bg-card rounded-3xl border border-border">
            ...
            <p className="text-sm font-semibold text-muted-foreground">
              No rankings yet. Play matches to climb!
            </p>
          </div>
        ) : (
          <div className="bg-card rounded-3xl border border-border overflow-hidden shadow-card">
            ...
            <div className="grid grid-cols-12 gap-2 px-4 py-3 border-b border-border bg-secondary/30 text-[9px] uppercase tracking-widest text-muted-foreground font-black">
              ...
              <div className="col-span-2 text-right" style={{ color: "var(--primary)" }}>
                Pts
              </div>
            </div>
            ...
            <div>
              {leaderboard.map((p: any, i: number) => {
                const isMe =
                  user &&
                  (p.user_id === user.id || (p.member_ids && p.member_ids.includes(user.id)));
                return (
                  <motion.div
                    key={p.user_id || i}
                  ...
                  >
                    ...
                    <div className="col-span-8 flex items-center gap-3">
                      <div
                        className="w-9 h-9 rounded-2xl flex items-center justify-center font-display font-black text-sm shrink-0 overflow-hidden"
                        style={{
                          background: isMe ? "var(--gradient-primary)" : "var(--secondary)",
                          color: isMe ? "white" : "var(--muted-foreground)",
                        }}
                      >
                        {p.logo ? (
                          <img
                    ...
                    <div className="col-span-2 text-right">
                      <span
                        ...
                        style={{ color: isMe ? "var(--primary)" : "var(--cta)" }}
                      >
...
function PodiumPillar({
  ...
  return (
    ...
    >
      ...
      {/* Name */}
      <div className="text-xs font-black text-foreground truncate w-full text-center mb-1 px-1">
        {p.team || "Unknown"}
      </div>
      ...
      <div
        ...
        style={{
          ...
          boxShadow: `inset 0 4px 20px ${cfg.bg}`,
        }}
      >
