    ...
    <div className="bg-background min-h-screen pb-24">
      ...
      <div className="bg-white rounded-b-[2rem] shadow-[0_4px_24px_oklch(0_0_0/0.04)] pt-6 pb-6 px-4 relative overflow-hidden z-10">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1 font-semibold">Stay updated on your matches</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {notifications.length === 0 ? (
            ...
            notifications.map((n, i) => (
              ...
              >
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 shadow-sm ${n.is_read ? 'bg-secondary text-muted-foreground' : 'bg-gradient-to-br from-primary to-[#d95a00] text-white'}`}>
                  <Bell className="w-5 h-5" />
                </div>
                <div className="flex-1 min-w-0 w-full">
                  <div className={`text-sm ${n.is_read ? 'text-muted-foreground' : 'text-foreground font-bold'}`}>{n.message}</div>
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-2 font-bold">{new Date(n.created_at).toLocaleString()}</div>
                  {n.action_type === 'tournament_request' && (
                    <div className="mt-4 flex gap-2 w-full sm:w-auto">
                      <Button onClick={() => handleResolve(n.action_data, 'approved')} className="flex-1 sm:flex-none h-10 rounded-xl font-bold bg-primary text-white shadow-primary">
                        <Check className="w-4 h-4 mr-1" /> Approve
                      </Button>
                      <Button variant="outline" onClick={() => handleResolve(n.action_data, 'rejected')} className="flex-1 sm:flex-none h-10 rounded-xl font-bold border-border shadow-sm hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30">
