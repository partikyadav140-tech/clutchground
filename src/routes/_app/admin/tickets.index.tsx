        ...
        <div className="relative z-10 flex items-center justify-between max-w-4xl mx-auto">
          ...
          <Link to="/admin">
            <Button
              variant="outline"
              size="sm"
              className="rounded-xl font-bold h-9 bg-card shadow-sm"
            >
              Back
            </Button>
          </Link>
            ...
            tickets.map((t) => (
              <Link key={t.id} to={`/admin/tickets/${t.id}` as any} className="block">
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="bg-card rounded-[1.5rem] border border-border hover:border-primary/40 hover:shadow-md transition-all p-5 flex items-center justify-between"
                >
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div
                        className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest ${t.status === "open" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                      >
                        {t.status}
                      </div>
                    </div>
                    <h3 className="font-display font-black text-lg text-foreground mb-1 group-hover:text-cta transition-colors line-clamp-1">
                      {t.subject}
                    </h3>
                    <div className="flex items-center gap-3 text-xs font-bold text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1">
                        <Clock className="w-3 h-3" /> {new Date(t.updated_at).toLocaleString()}
                      </span>
                    </div>
                  </div>
