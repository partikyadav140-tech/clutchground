        ...
        {!selectedTournamentId ? (
              ...
              {displayedTournaments.length === 0 ? (
                ...
                displayedTournaments.map((t: any, i: number) => {
                  ...
                  return (
                    ...
                    >
                      <div className="font-display font-black text-lg text-foreground group-hover:text-primary transition-colors line-clamp-1 mb-1.5">{t.title}</div>
                      ...
                      <div className="mt-auto flex items-center justify-between pt-4 border-t border-border/50">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground group-hover:text-primary group-hover:translate-x-1 transition-all">View &rarr;</span>
                      </div>
                    </motion.button>
                  );
                })
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-[1.5rem] border border-border shadow-sm p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h2 className="font-display text-2xl font-black text-foreground">{selectedTournament?.title}</h2>
                <div className="text-sm text-muted-foreground mt-1 font-semibold">Showing all confirmed registrations.</div>
              </div>
              <Button 
                variant="outline" 
            ...
            <div className="space-y-4">
              {filteredRegistrations.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-[1.5rem] border border-border shadow-sm">
                  ...
                  <p className="text-foreground font-semibold">No teams or players registered yet.</p>
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="p-5 grid sm:grid-cols-12 gap-6">
                        <div className="sm:col-span-4 space-y-4">
                          <div>
                            <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2 ml-1">Contact Details</div>
                            <div className="bg-secondary/50 rounded-xl p-3 border border-border space-y-2">
                              <div className="truncate text-xs font-semibold text-muted-foreground">
                                Email: <span className="text-foreground font-mono">{r.contact_email}</span>
                              </div>
                              <div className="truncate text-xs font-semibold text-muted-foreground">
                                Phone: <span className="text-foreground font-mono">{r.contact_phone}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="sm:col-span-8">
                          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2 ml-1">Squad Roster ({players.length} Players)</div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {players.map((p: any, idx: number) => (
                              <div key={idx} className="flex gap-3 items-center bg-white rounded-xl p-2.5 border border-border shadow-sm">
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="font-bold text-sm truncate text-foreground leading-tight">{p.ign}</div>
