              ...
              >
                <div className="p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border/50 pb-4 mb-4">
                    <div>
                      <h3 className="font-display font-black text-foreground text-lg mb-1">{m.name}</h3>
                      <p className="text-xs font-semibold text-muted-foreground bg-secondary/50 px-2.5 py-1 rounded-md inline-block">{m.email}</p>
                    </div>
                    <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest self-start sm:self-center bg-white border border-border px-2.5 py-1 rounded-md shadow-sm">
                  ...
                  <div className="mt-5 flex justify-end">
                    <Button asChild variant="outline" className="rounded-xl font-bold border-primary/20 text-primary hover:bg-primary/5 h-10 px-5">
                      <a href={`mailto:${m.email}`}><Mail className="w-4 h-4 mr-2" /> Reply via Email</a>
                    </Button>
                  </div>
                </div>
              </motion.div>
