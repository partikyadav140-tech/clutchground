...
function AdminSiteSettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"general" | "payments" | "content">("general");
  ...
  return (
    <div className="bg-background min-h-screen pb-2">
      ...
      <div className="px-4 pt-5 space-y-4 max-w-2xl mx-auto">
        {/* Tab Selector */}
        <div className="flex gap-1.5 overflow-x-auto hide-scrollbar pb-1 border-b border-border/40">
          {[
            { key: "general", label: "General & Safety", icon: Settings },
            { key: "payments", label: "Payments & Gateway", icon: CreditCard },
            { key: "content", label: "Content & Assets", icon: Image },
          ].map((t) => {
            const Icon = t.icon;
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full font-bold text-xs whitespace-nowrap flex items-center gap-2 transition-all shrink-0 ${
                  active
