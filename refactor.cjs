...
code = code.replace(/(tx|db)\.prepare\((.*?)\)\.(get|all|run)\((.*?)\)/g, 'await $1.prepare($2).$3($4)');

// 3. Fix cases where stmt is assigned to a variable and called later
// e.g. const stmt = db.prepare('...'); stmt.run();
const varsToAwait = ['stmt', 'checkStmt', 'insertStmt', 'userStmt', 'old', 'insertNotif', 'insertMember', 'addPrize', 'insertAdmin', 'insertTournament', 'countStmt'];
