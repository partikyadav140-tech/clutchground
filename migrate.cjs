...
const pool = new Pool({ 
  connectionString: process.env.DATABASE_URL || "postgresql://postgres.mvkvdxphxzvviaunqmlb:partikbahi09@aws-1-ap-southeast-1.pooler.supabase.com:6543/postgres",
...
async function migrate() {
  try {
    ...
    await pool.query("ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS tournament_type TEXT DEFAULT 'battle_royale'");
    console.log("✓ tournaments tournament_type added");
    
    await pool.query("ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS entry_fee INTEGER DEFAULT 0");
    console.log("✓ tournaments entry_fee added");
    
    await pool.query("ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS prize_pool INTEGER DEFAULT 0");
    console.log("✓ tournaments prize_pool added");
    
    await pool.query("ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS open_slots INTEGER DEFAULT 0");
